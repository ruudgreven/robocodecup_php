...
Flight::route('GET /round/@sRoundId/ranking.json', function($sRoundId){
    ...
    //Get all rounds
    $sQuery = "SELECT pool.id AS pool_id, pool.name AS pool_name, team.id as team_id, team.name AS team_name, AVG(battlescore.totalscore) AS totalscore, AVG(battlescore.survivalscore) AS survivalscore, AVG(battlescore.survivalbonus) AS survivalbonus, AVG(battlescore.bulletdamage) AS bulletdamage, AVG(battlescore.bulletbonus) AS bulletbonus, AVG(battlescore.ramdamage) AS ramdamage, AVG(battlescore.rambonus) AS rambonus, AVG(battlescore.firsts) AS firsts, AVG(battlescore.seconds) AS seconds, AVG(battlescore.thirds) AS thirds, COUNT(*) AS totalbattles FROM battle, battlescore, team, pool WHERE battle.competition_id = team.competition_id = battlescore.competition_id = pool.competition_id = " . COMPETITION_ID . " AND battlescore.battle_id = battle.id AND battlescore.team_id = team.id AND battlescore.pool_id = pool.id AND round_number=" . $sRoundId. " AND battle.official = 1 GROUP BY team_id, pool_id ORDER BY SUM(battlescore.totalscore) DESC;";
    $oResult = $oDbHelper->executeQuery($sQuery);
